...
for kv in "${ARRAY[@]}" ; do
    KEY=${kv%%:*}
    VALUE=${kv#*:}
    env GOOS=$KEY GOARCH=$VALUE go build -trimpath -ldflags "-s -w -X main.built=`date -u +.%Y%m%d.%H%M%S` -X main.version=0.0.3" -o ./jamsync-build/public/jam cmd/client/main.go 
